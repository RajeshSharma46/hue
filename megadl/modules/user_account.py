    ...
    if todownfile.media is None:
        try:
            ...
            if os.path.isdir(dl_path):
                ...
                await megaupmsg.edit(f"**Successfully Uploaded To Mega.nz** \n\n**Link:** `{link}` \n\n**Powered by @HEY_HABIBI**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📥 Mega.nz Link 📥", url=f"{link}")]]))
...
async def importurlf(_, message: Message):
    ...
    try:
        ...
        try:
            ...
            await message.reply_text(f"**Successfully Imported 😌** \n\n**Link:** `{imported_link}` \n\n**Powered by @HEY_HABIBI**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📥 Imported Link 📥", url=f"{imported_link}")]]))
        except Exception as e:
